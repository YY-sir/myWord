...
+ (TTTipView *)tipViewWithType:(TTTipViewType)type blockUserInteract:(BOOL)isBlock {
    
    CGRect bgFrame = [UIScreen mainScreen].bounds;
    TTTipView *tipView = [[TTTipView alloc] initWithFrame:bgFrame type:type];
    tipView.userInteractionEnabled = isBlock;
    return tipView;
}
    ...
    if (self.shouldAutoDismiss) {
        if (self.tipType == TTTipViewTypeTextHint) {
            ...
            if (self.textLabel.attributedText) {
                dismissDuration = self.textLabel.attributedText.length * 0.16;
            }
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    switch (_tipType) {
        case TTTipViewTypeTextHint: {
            self.contentHeightConstraint.constant = 5;
            [self layoutIfNeeded];
            CGSize size = [self.textLabel sizeThatFits:CGSizeMake(self.textLabel.frame.size.width, MAXFLOAT)];
            //NSLog(@"%f %f", size.width, size.height);
